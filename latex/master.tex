...
\begin{equation}
T_1 / T_2 = (I_2 / I_1)^{(1/5)} \quad \text{(see Eq. \ref{eq:ste})}
\end{equation}
...
Under isometric scaling at constant density, all dynamical timescales in a system --- orbital periods, oscillation periods, free-fall times --- scale with the linear scale factor $k$. By Premise 3, clock rates scale identically, because every clock is a dynamical system. By Premise 1, $k = (I_2/I_1)^{(1/5)}$. Therefore:
\begin{equation}
T_1/T_2 = k_2/k_1 = (I_2/I_1)^{(1/5)} \quad \text{(see Eq. \ref{eq:ste})}
\end{equation}
...
\begin{equation}
T_1/T_2 = (M_2/M_1)^{(1/3)} \quad \text{(see Eq. \ref{eq:linear})}
\end{equation}

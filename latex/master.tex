...
\cite{degerlia2025universe, degerlia2025}

Inertia is relativistic. Meaning that without a comparator, any measurement of it would have no meaning. Something with a moment of inertia $1 \times 10^{20}$ kg/m$^2$ would be very difficult to rotate. But not more difficult than something with $1 \times 10^{30}$ kg/m$^2$.
...
\begin{align}
l' &= kl \\
&\quad \text{where } l \text{ is the characteristic length of the system,} \notag \\
&\quad \text{and } k \text{ is the lineart scale factor} \notag \\
m' &= k^3 \\
&\quad \text{where mass is } m \text{ and } k \text{ is linear scale factor} \notag \\
i' &= k^5i \\
&\quad \text{where } i \text{ is the moment of inertia about a selected axis;} \notag \\
&\quad k \text{ is the linear scale factor.} \notag \\
i_{lin}' &= m' = k^3m \\
&\quad \text{where } i_{lin} \text{ is linear inertia (mass);} \notag \\
&\quad k \text{ is the limear scale factor.} \notag
\end{align}
...
\subsection{The space-time equivalene (STE)}
...
t_1/t_2=(i_2/i_1)^{(1/5)}
\end{equation}
where $i$ is moment of inertia about the axis of observation; $t$ is the clock time of the respective system system 1 and 2.
...
T_1 / T_2 = (I_2 / I_1)^{(1/5)}
\end{equation}
Where $T_1$ and $T_2$ are the clock rates in two systems, and $I_1$ and $I_2$ are their moments of inertia about the axis connecting the two systems. The derivation of this formula from first principles is given in Section 3. For linear (non-rotational) systems, inertia reduces to mass and the exponent becomes 1/3.
...
Consider two systems that are isometrically scaled copies of each other: identical in shape and composition, differing only in size. Let all lengths scale by a factor $\lambda$. At constant density:
\begin{itemize}
\item Mass scales as volume: $M \propto \lambda^3$
\item Radius scales as length: $R \propto \lambda$
\item Moment of inertia: $I = kMR^2 \propto \lambda^3 \cdot \lambda^2 = \lambda^5$
\end{itemize}
The moment of inertia scales as the fifth power of the characteristic length. Inverting:
\begin{equation}
\lambda_2/\lambda_1 = (I_2/I_1)^{(1/5)}
\end{equation}
...
Under isometric scaling at constant density, all dynamical timescales in a system --- orbital periods, oscillation periods, free-fall times --- scale with the characteristic length $\lambda$. By Premise 3, clock rates scale identically, because every clock is a dynamical system. By Premise 1, $\lambda = (I_2/I_1)^{(1/5)}$. Therefore:
\begin{equation}
T_1/T_2 = \lambda_2/\lambda_1 = (I_2/I_1)^{(1/5)}
\end{equation}
...
The square root arises from Einstein's choice of mathematical framework. General relativity is built on Riemannian geometry, in which the fundamental object is the metric tensor $g_{\mu\nu}$. The metric tensor defines a quadratic form:
\begin{equation}
ds^2 = g_{\mu\nu} dx^\mu dx^\nu
\end{equation}
...
Because the metric outputs $ds^2$ (the squared interval), all solutions to Einstein's field equations, including Schwarzschild's, inherit this quadratic form. The Schwarzschild solution for a stationary clock is:
\begin{equation}
d\tau^2 = (1 - r_s/r) dt^2
\end{equation}

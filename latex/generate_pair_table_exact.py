...
10 significant digits, compact e-style notation, row groups preserved."""
...
def emit_row(label, values):
    ...
    # Show full source precision (30 sig figs) in Case 3 (col 2) and STE
    # Example (col 5). Other columns stay at 10.
    HI_PREC_COLS = {2, 5}
    formatted = []
    for i, v in enumerate(values):
        prec = 30 if i in HI_PREC_COLS else 10
        formatted.append(r'{\ttfamily ' + fmt_round_pad(v, prec, prec) + r'}')
...
def emit_table_header(caption, label):
    ...
    print(r'\begin{tabular}{@{} l l l l l l l @{}}')
    print(r'\toprule')
...
def main():
    ...
    # Reorder the last group: m_1/m_2, sqrt(m_1/m_2), r_2/r_1, sqrt(r_2/r_1).
    # All four rows come directly from source.
    if len(groups) > 5:
